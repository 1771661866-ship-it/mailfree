...
export async function handleEmailEvent(message, env, ctx) {
  ...
  try {
    ...
    } catch (e) { console.error('R2 put failed:', e); objectKey = ''; }
    ...
    await DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(mailboxId, sender, String(toAddrs || ''), subject || '(无主题)', verificationCode || null, preview || null, textContent || '', htmlContent || '', objectKey ? 'mail-eml' : '', objectKey || '').run();
  } catch (err) {
